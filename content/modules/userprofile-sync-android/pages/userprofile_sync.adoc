...
Couchbase Mobile introduces a brand new websockets based
...
The user profile view is _automatically updated_ everytime the profile information changes in the underlying database
...
Clone the link:https://github.com/couchbaselabs/userprofile-couchbase-mobile-android/tree/sync[*_sync_* branch] of the `User Profile Demo` project from GitHub.
...
If have followed along the tutorial on link:[Query Basics], you can skip this section and proceed to the <<Backend Installation>>. section.
...
A value of _false_  which implies that data is only pulled from the Sync Gateway.
...
----
<1> Record the revision Id of the document.
You will need this when you update the document
<2> If you had updated an image via the mobile app, you should see an * "_attachments"* property.
...
You can learn more about how image Blob types are mapped to attachments link:[here].
...
----
<1> I updated the university field via the REST API.
...
NOTE: From <<Exercise 2>> above, you observed that changes made on the Sync Gateway are propagated to the Couchbase Lite clients.
However, if you tried to update the attachment / image using the link:[Attachments REST API], you would not see the image getting updated on the client side.
This is a link:[known issue] in 2.0 and should be fixed in the next release.


== Handling Conflicts during Data Synchronization


Data conflicts are inevtiable in an environment where you can potentially have multiple writes updating the same data concurrently.
